...
TARGET VesselClass::Greatest_Threat(ThreatType threat) const
{
    if (*this == VESSEL_SS) {
        ...
        if ((threat
             & (THREAT_GROUND | THREAT_POWER | THREAT_FACTORIES | THREAT_TIBERIUM | THREAT_BASE_DEFENSE | THREAT_BOATS))
            == 0) {
...
int VesselClass::Mission_Retreat(void)
{
    ...
    case PICK_RETREAT_POINT:
        IsALoaner = true;
        if (!Target_Legal(NavCom)) {
            //				CELL cell = Map.Calculated_Cell(House->Control.Edge, (Team.Is_Valid()) ? Team->Class->Origin : -1,
            //-1, Class->Speed);
                    ...
                    if (vessel != NULL) {
                        ...
                        */
                        ...
                        if (vessel->Unlimbo(coord, dir)) {
                            ...
                            //						vessel->Strength = Fixed_To_Cardinal(vessel->Class->MaxStrength,
                            //strength);
